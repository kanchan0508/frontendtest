...
export default function DashboardHeader({
  ...
  return (
    <>
      <header className="sm:hidden h-[64px] flex items-center justify-between px-4 bg-white border-b border-[#eaecf0] sticky top-0 z-[80]">
        <div className="flex items-center gap-2 min-w-0">
          <button
            type="button"
            onClick={() => window.dispatchEvent(new Event("dashboard:open-sidebar"))}
            className="flex h-8 w-8 items-center justify-center rounded-[16px]"
            aria-label="Open menu"
          >
            <div className="h-6 w-6 bg-[url(https://codia-f2c.s3.us-west-1.amazonaws.com/image/2026-02-28/AP89NofDaV.png)] bg-cover bg-no-repeat" />
          </button>
          <div className="min-w-0">
            {subtitle && (
              <p className="truncate font-mulish text-[12px] leading-[15px] text-[#363c42]">
                {subtitle}
              </p>
            )}
            <h2 className="truncate font-arima text-[20px] font-extrabold leading-[32.66px] uppercase text-[#363c42]">
              {title}
            </h2>
          </div>
        </div>
        <div ref={mobileNotificationRef} className="relative">
          <button
            ...
            className="h-8 w-8 rounded-[16px] bg-[url(https://codia-f2c.s3.us-west-1.amazonaws.com/image/2026-02-28/7gicVSjxfK.png)] bg-cover bg-no-repeat"
          ...
          {isNotificationOpen && (
            <>
              ...
              />
              <div className="fixed left-3 right-3 top-[72px] z-[100] flex max-h-[75vh] flex-col gap-[10px] rounded-[12px] bg-[#fff] p-[14px] shadow-[0_18px_42px_0_rgba(9,15,46,0.25)]">
                ...
                <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">
                  {NOTIFICATIONS.map((notification, index) => (
                    <div
                      key={`${notification.title}-mobile-${index}`}
                      className={`flex gap-[8px] self-stretch bg-[#fff] px-0 py-[12px] ${
                        index < NOTIFICATIONS.length - 1
                          ? "border-b border-solid border-b-[#f0e6f3]"
                          : ""
                      }`}
                    >
                      <div className="flex grow basis-0 gap-[12px]">
                        <div className="flex grow basis-0 flex-col gap-[4px]">
                          ...
                          <span className="flex w-full justify-start overflow-hidden font-mulish text-[12px] font-normal leading-[16px] tracking-[0.2px] text-left text-[#4a4a4a]">
      ...
      <header className="hidden sm:flex h-[100px] items-center justify-between px-10 bg-white border-b border-[#eaecf0] sticky top-0 z-[19]">
